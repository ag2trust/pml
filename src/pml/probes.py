...
def load_probes(
    ...
    for source in sources:
        ...
        if probe_id in probes:
            diagnostics.append(
                Diagnostic(f"{source}:probe", "duplicate-probe", f"probe '{probe_id}' is already defined")
            )
        else:
        ...
        if obligation is None:
            diagnostics.append(
                Diagnostic(f"{source}:verifies", "undefined-reference", f"unknown obligation '{probe['verifies']}'")
            )
        elif bindings is not None:
            configured = verification_plan(bindings, obligation).get("probes", {})
            if probe_id not in configured:
                diagnostics.append(
                    Diagnostic(f"{source}:probe", "unbound-probe", "probe has no approved coverage binding")
                )
        ...
        for index, step in enumerate(probe["steps"]):
            ...
            for variable in VARIABLE.findall(serialized):
                if variable not in captured:
                    diagnostics.append(
                        Diagnostic(f"{source}:steps[{index}]", "undefined-variable", f"variable '{variable}' is used before capture")
                    )
            actor = step.get("as")
            if actor is not None and actor not in actors:
                diagnostics.append(
                    Diagnostic(f"{source}:steps[{index}].as", "undefined-reference", f"unknown actor '{actor}'")
                )
            for variable in step.get("capture", {}):
                if variable in captured:
                    diagnostics.append(
                        Diagnostic(f"{source}:steps[{index}].capture.{variable}", "duplicate-capture", f"variable '{variable}' is already captured")
                    )
