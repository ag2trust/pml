...
the product-local `architecture` map and their generated state belongs under
`.pml/architecture/`; they never contribute to product status. Each decision must be
referenced by a feature or component. There are no `applies_to`, `supports`, inline,
or recursive architecture constructs.
