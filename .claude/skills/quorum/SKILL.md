...
**Why this matters:** if an interactive session completes implementation while the Quorum
task remains `working`, the daemon may provision a second worker for the same task. The
cancel-then-review-only protocol prevents this class of conflict.
