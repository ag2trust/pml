...
from pml.compiled_model import CompiledBehavior, CompiledFeature, CompiledModel, CompiledSignal
...
def test_schema_accepts_every_v1_variant() -> None:
    ...
    model["behaviors"][0]["outcome"] = {"kind": "one_of", "exclusivity_obligation": behavior + ".outcome", "cases": [{"id": "handled", "obligation": behavior + ".outcome.handled", "statement": "The note is handled.", "signal": "note_done"}]}  # type: ignore[index]
    ...
    model["obligations"] = [
        ...
        {"id": behavior + ".completion", "node": behavior, "kind": "completion", "definition": {"outcomes": [behavior + ".outcome.handled"], "failures": [behavior + ".failures.rejected"]}},
        {"id": behavior + ".outcome", "node": behavior, "kind": "outcome_exclusivity", "definition": {"alternatives": [behavior + ".outcome.handled"]}},
